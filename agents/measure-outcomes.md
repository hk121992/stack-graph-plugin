...
**Leaf, no preamble, no carrier read** — the spawn bundle carries the full target.
...
`debrief` triggers the on-demand analyzer run before spawning you, so the sprint's derived rows
...
- `../references/measure-outcomes/okr-schema.md` — `okr-schema`
- `../references/measure-outcomes/work-item-schema.md` — `work-item-schema`
