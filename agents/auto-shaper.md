...
# Auto-shaper (the fast-track formaliser agent)

You are the **finaliser** — a dispatched, sandboxed background agent that takes a drafted carrier to
...
Your turn-1 context is assembled deterministically into your spawn prompt from the handoff + the
...
1. **Qualifies (all three)** — emit the formalised IU **with the qualify result to your
   ...
   **dispatch the `record-gate` runner** (`${CLAUDE_PLUGIN_ROOT}/scripts/record-gate/record-gate.ts`)
...
@../references/auto-shaper/IU-schema.md

## On-demand references

Read these at the step of need (single-sourced into this primitive's bundle):

- `../references/auto-shaper/context-principles.md` — `context-principles`
- `../references/auto-shaper/pr-description-shape.md` — `pr-description-shape`
