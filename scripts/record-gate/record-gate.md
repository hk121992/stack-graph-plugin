...
and the full hash-chain definition — are specified in your imported `gate-model`; read `IU-schema` and
...
## Invocation — you are an executable runner, dispatched

You are a deterministic runner (`primitive: script`), not a session procedure. A firing node enacts
a gate write by **dispatching** you over Bash — never by hand-executing the precondition-check and
the chain append in its own session:

```
bun ${CLAUDE_PLUGIN_ROOT}/scripts/record-gate/record-gate.ts \
...
1. **Compute the chain link.** Append is a hash-**chain** link, not a standalone per-entry hash:
   ...
   single-writer guard imports — `scripts/record-gate/deps/canonical.ts`.
...
- A **rejection prints no tag** — the append reaches only past every precondition, so a refused write is
  ...
  written.
- The scanner accepts the tag **only with executed-runner provenance** — it must ride the `tool_result`
  of a Bash `tool_use` whose command **executed this runner** (executed-argv-anchored, not a substring).
  A prose echo, a mention-only command, a file-read of this reference or the runner source, or a replay
  through a non-runner tool_result is **refused** (counted, never derived). You emit the honest tag; the
