...
`checkpoint-manifest` instance (external, pointer-only, like [experience-contract-schema](experience-contract-schema.md)).
...
- `clean_room` — the hermetic container pin: the agent-CLI version + OS deps the turns run under, and the
  ...
  [§inv-clean-room](checkpoint-driver.md#inv-clean-room) for the why — a bare-host run is forbidden and a
...
[§inv-scrub](checkpoint-driver.md#inv-scrub).
...
the reply boundary — see [§secret-guard](checkpoint-driver.md#secret-guard).
...
[§inv-burn-in](checkpoint-driver.md#inv-burn-in) for the miss-probability math.
...
- **The verdict records** the mode emits for a FAIL → [findings-schema](findings-schema.md).
- **The sibling experience contract** authored at design and graded at verify → [experience-contract-schema](experience-contract-schema.md).
