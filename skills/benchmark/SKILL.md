...
When run in an isolated child context, act as the autonomous performance-measurement role. Measure
the product's performance, compare it against a **stored baseline**, and return a **regression
verdict plus a `benchmark.perf` trend measurement**. Run unattended to a verdict — no operator
turn. This role measures **performance**; other baseline roles own their declared subjects.
...
Render as branches of this one role — same measurement, different output.
