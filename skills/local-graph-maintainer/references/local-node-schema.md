...
vendored [node & edge schema](../../../references/node-edge-schema.md): the factory authors a node and `generate` projects it to a
...
- **The shared node/edge shape** — the frontmatter superset, the edge vocabulary + cyclic rules, the goals block, a reference's frontmatter → [node-edge-schema](../../../references/node-edge-schema.md).
- **The path-resolution seam** the binding read resolves against → [bindings-contract](../../../references/bindings-contract.md).
