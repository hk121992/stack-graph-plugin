...
[checkpoint-manifest](../../../references/checkpoint-manifest-schema.md) content differs. This reference fixes the do-this;
...
product-specific members, registered on the manifest — [§operators](../../../references/checkpoint-manifest-schema.md#operators),
...
([§scenarios](../../../references/checkpoint-manifest-schema.md#scenarios)) so a gate operator sees the weakness rather than
...
- Emit a record per FAIL to [findings-schema](../../../references/findings-schema.md) so it consolidates at the caller's panel
...
- The verdict records emitted for a FAIL → [findings-schema](../../../references/findings-schema.md).
