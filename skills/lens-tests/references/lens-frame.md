...
Return findings conforming exactly to [findings-schema](../../../references/findings-schema.md), compact tier (§Return tiers — omit `why_it_matters` and `evidence`): severity per [severity-scale](../../../references/severity-scale.md), confidence per [confidence-anchors](../../../references/confidence-anchors.md) applied as a self-rubric with its suppression behaviour — drop what you cannot anchor, never pad. For a `doc` or `plan` target, the doc location/section stands in for `file`/`line`. No operator-facing prose, no summary narrative. An empty finding set is a valid return, not a failure.
