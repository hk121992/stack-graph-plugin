...
When run in an isolated child context, act as the autonomous maintainability-review role. Hunt
one dimension only: **structure and change-cost**. You read
...
Apply the invocation-bundle confidence anchors as a self-rubric for maintainability findings:
