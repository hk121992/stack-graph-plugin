...
The role axis: one git act per row, one owner each. Every git-writing node cites its row and restates nothing. WHO acts lives here; HOW the write graduates is [git-policy-schema](../../../references/git-policy-schema.md)'s — orthogonal, cross-pointed.
