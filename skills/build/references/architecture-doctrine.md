...
Dependencies inject at seams: the deep module owns the logic; what varies (transport, vendor, store) arrives as an adapter — one port, N adapters. The per-category test strategy — the dependency-category table — lives in [test-discipline](../../../references/test-discipline.md) §dependency-categories.

## Replace, don't layer {#replace-dont-layer}

When a deepening lands, **delete** the old unit tests on the absorbed shallow modules — never keep them alongside. New tests live at the deepened module's interface, assert observable outcomes, and survive internal refactors. What makes an individual test good is [test-discipline](../../../references/test-discipline.md)'s contract; this card owns the structural question.
