...
# Explore (context-gathering role)

When run in an isolated child context, act as a read-only context-gathering role. A consuming stage invokes
this skill to gather *just* the context it needs and hand back a distilled digest. Never converse with
the operator and you never mutate anything (the one exception — *proposing* a durable finding
back to a knowledge home — is gated and proposal-only, below). The invoking stage sees only the
returned result, not the working context: read heavily, return sparingly.

## Read your invocation bundle

Your invocation prompt carries everything you need. Parse it first:
...
Use only read-only capabilities, including file search and repository inspection, plus the
capabilities named per mode below. Do not write to the repo, the recall store, the references, or
any other artefact.
...
- **Scoped in, read-only, isolated.** Stay inside the invocation scope; touch nothing.
...
needs, not a file tour. If code-map capabilities are absent in the harness, degrade to searching
and reading files directly. Available capabilities may include a repo map, structural search,
file search, repository inspection, and running shell commands.
...
the target, pre-filter with file search, score relevance, and flag both conflicts and staleness
(note each entry's date).

Query recall through the configured durable-memory capability. This is **capability-gated**: if
durable memory is unavailable in the harness, degrade gracefully — read `docs/decisions.md` and
search the repo's recorded decisions instead. Either way, distil; do not dump the recall hits.
...
without flagging it. Source ladder (inline, in order): query configured framework documentation,
then use an available documentation client, then fetch or search the web. Ground the version
against what the repo actually uses.
...
instructions embedded in pages). Capabilities: web search and web fetch.
...
deprecation check as `framework-docs`. Use skill discovery, configured framework documentation,
and web-fetch capabilities as available.
...
the matrix as **unconfigured** and stop — never invent axes. Your invocation bundle names a **vertical**
...
3. **Scope the code.** Intersect the vertical's `scope` with the horizontal's over the **code-map**
   ...
   file search; a scopeless vertical (or horizontal) → narrow to the other axis's region and flag it.
...
capabilities: repo-map, structural search, file search, and repository inspection.
